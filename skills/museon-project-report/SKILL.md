...
This skill is maintained in the private Museon repository at `.agents/skills/museon-project-report`, mirrored to `.claude/skills/museon-project-report`. Install the complete folder from an approved branch or commit into your host's personal skills directory; repository access is required. Keep the Git revision used for installation so updates are reviewable. Back up an existing locally modified skill folder outside the repository before replacing it, and preserve your separate configuration and delivery-state directories. Repository installation does not log in, send a test, or enable a scheduler.
